		...
		if (Input.touchCount > 0)
		{
			if (Input.GetTouch(0).phase == TouchPhase.Began)
			{
				...
				if (newPos.x < limitPos.x && newPos.x > -limitPos.x)
				{
					...
					StartCoroutine(CameraMove(new Vector3(camera.transform.position.x, newPos.y, UIManager.instance.cameraZpos)));
				}
				...
				if (newPos.y < limitPos.y && newPos.y > -limitPos.y)
				{
					...
					StartCoroutine(CameraMove(new Vector3(newPos.x, camera.transform.position.y, UIManager.instance.cameraZpos)));
				}
			}
		}
	}
