...
public class CameraChase : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		temp = new Vector3(Mathf.Lerp(transform.position.x, chaseObject.position.x - fixX, speed),
			Mathf.Lerp(transform.position.y, chaseObject.position.y - fixY, speed), -110);

		if (temp.x < limitX && temp.x > -limitX)
		{
			transform.position = new Vector3(temp.x, transform.position.y, -110);
		}

		if (temp.y < limitY && temp.y > -limitY)
		{
			transform.position = new Vector3(transform.position.x, temp.y, -110);
		}
	}
}
